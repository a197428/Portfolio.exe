...
| Accepted | Role is part of the URL (`?role=frontend` / `?role=ai`) | Outreach links land on the right lens; URL beats the saved preference, `replace` avoids history noise. |
